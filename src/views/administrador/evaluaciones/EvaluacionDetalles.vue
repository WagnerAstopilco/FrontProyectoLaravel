<template>
    <div class="container">
        <div class="card p-4">
            ...
            <div v-if="evaluation">
                <!--  -->
                <form @submit.prevent="updateEvaluation()" class="w-50">
                    <fieldset>
                        <div class="form-group d-flex flex-column">
                            <label for="title">Titulo</label>
                            <input type="text" id="title" class="form-control p-2" v-model="evaluation.title" placeholder="Titulo de la evaluación" :readonly="!isEditing"/>
                        </div>
                        <div class="form-group d-flex flex-column">
                            <label for="start_date">Fecha de inicio</label>
                            <input type="date" id="start_date" class="form-control p-2" v-model="evaluation.start_date" placeholder="Titulo de la evaluación" :readonly="!isEditing"/>
                        </div>
                        <div class="form-group d-flex flex-column">
                            <label for="end_date">Fecha de fin</label>
                            <input type="date" id="end_date" class="form-control p-2" v-model="evaluation.end_date" placeholder="Titulo de la evaluación" :readonly="!isEditing"/>
                        </div>
                        <div class="form-group d-flex flex-column">
                            <label for="duration">Duración</label>
                            <input type="time" id="duration" class="form-control p-2" v-model="evaluation.duration" placeholder="Titulo de la evaluación" :readonly="!isEditing"/>
                        </div>
                        <div class="form-group d-flex flex-column">
                            <label for="attempts_allowed">Intentos permitidos</label>
                            <input type="number" id="attempts_allowed" class="form-control p-2" v-model="evaluation.attempts_allowed" placeholder="Titulo de la evaluación" :readonly="!isEditing"/>
                        </div>
                        <div class="form-group">
                                <label for="state">Estado</label>
                                <select name="state" v-model="evaluation.state" class="form-control p-2" :disabled="!isEditing">
                    ...
                    <div>
                        ...
                        <div class="modal-dialog">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary" data-bs-dismiss="modal" @click="createNewQuestion()">Crear</button>
                            </div>
                            </div>
                        </div>
                        </div>
                            ...
                            <table id="questionsTable" class="table table-striped">
                                ...
                                <tbody>
                                    <tr v-for="question in evaluation.questions" :key="question.id">
                                        ...
                                        <td><button @click="deleteQuestion(question)">Eliminar</button></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
    ...
    methods: {
        ...
        async createNewQuestion() {
            try {
                ...
                // Si es de opción múltiple, primero validamos las opciones
                let optionsToSend = [];
                ...
                // Solo ahora creamos la pregunta
                ...
                // Si es de opción múltiple, asociamos las opciones con la pregunta y las enviamos
                if (this.newQuestion.type === 'opcion multiple') {
