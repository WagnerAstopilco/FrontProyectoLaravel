            ...
            <div class="card-body w-lg-80 w-md-90 w-100 mx-auto">
                <form @submit.prevent="createTrainer" >
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="certifications" class="form-control" v-model="newTrainer.certifications" placeholder="Certificaciones del capacitador"></textarea> 
                    </div>
                    <div class="form-group">
    ...
    methods:{
        ...
        async createTrainer(){
            ...
            try{
                ...
                Object.entries(this.user).map(([key, value]) => formDataTrainer.append(key, value));
                let response =await UserService.postUser(formDataTrainer);
                const idNewUser=response.data.data.id;
                this.newTrainer.user_id=idNewUser;
                response=await TrainerService.postTrainer(this.newTrainer);
                const idNewTrainer=response.data.data.id;
                this.$router.push({name: 'CapacitadorDetallesVer',params: { idcapacitador: idNewTrainer },});
            }catch (err) {
