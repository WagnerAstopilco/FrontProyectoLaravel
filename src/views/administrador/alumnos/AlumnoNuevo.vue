    ...
    methods:{
        ...
        async createStudent(){
            ...
            try{
                ...
                Object.entries(this.user).map(([key, value]) => formDataStudent.append(key, value));
                const response =await UserService.postUser(formDataStudent);
                const idNewStudent=response.data.data.id;
                this.$router.push({name: 'AlumnoDetallesVer',params: { idalumno: idNewStudent },});
            }catch (err) {
                if (err.response && err.response.data && err.response.data.errors) {
                    this.error = Object.values(err.response.data.errors).flat().join(" ");
                } else {
                    this.error = "Ocurrió un error inesperado al crear el alumno.";
                }
            }finally{
                this.loading=false;
            }
